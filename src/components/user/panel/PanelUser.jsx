...
const PanelUser = (props) => {
  ...
  React.useEffect(() => {
    ...
    const traerInfo = async () => {
      try {
        const response = await fetch(process.env.SERVER_URL+`/api/user/qty/${props.user.dni}`, {
          method: "GET",
        ...
        if(info.mascotas===0) {
          ...
          const res = await fetch(process.env.SERVER_URL+`/api/user/pacientes/mascotas/${props.user.dni}`, {
            method: "GET",
            signal: abortCont.signal,
          });
